...
def handle(event, context):
    ...
    stabilitywarrants_sg_csv = tempfile.NamedTemporaryFile(delete=False)
    stabilitywarrants_sg_csv.close()
    with open(stabilitywarrants_sg_csv.name, 'w') as csv_file:
    ...
    utils.upload_file(stabilitywarrants_sg_csv.name, 'raw/sg/csv/%Y/%m/stabilitywarrants-sg-%Y-%m-%d.csv')
