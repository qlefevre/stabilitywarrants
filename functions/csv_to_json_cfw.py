...
def handle(event, context):
    ...
    cappedflooredwarrants_cf_csv = download_file(
        'cf/sg/%Y/%m/cappedflooredwarrants-sg-%Y-%m-%d.csv')
    cappedflooredwarrants_cf_json = create_temp_file()
    with open(cappedflooredwarrants_cf_csv, 'r') as file_csv:
        fieldnames = file_csv.readline().strip().split(';')
        file_csv.seek(0)
        reader = csv.DictReader(file_csv, fieldnames, delimiter=';')

        with open(cappedflooredwarrants_cf_json, 'w', encoding='utf8') as file_json:
            file_json.write('[')
